...
//		// 校验用户名密码
		...
		if (user != null) {
			if (Global.NO.equals(user.getIsUseable())){
				throw new AuthenticationException("msg:该已帐号状态不可用");
			}
...
//
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		User user = null;//getSystemService().getUserByLoginName(principal.getLoginName());
		if (user != null) {
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			List<Menu> list = null ;//UserUtils.getMenuList();
			for (Menu menu : list){
