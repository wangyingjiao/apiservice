package com.thinkgem.jeesite.common.pushMessage;
...
public class PushMessageUtil {
    ...
    public  static int pushMessage(MessageInfo messageInfo) {
        ...
        // 推送目标
        pushRequest.setAppKey(messageInfo.getAppKey());
        pushRequest.setTarget("ACCOUNT"); //推送目标: DEVICE:按设备推送 ALIAS : 按别名推送 ACCOUNT:按帐号推送  TAG:按标签推送; ALL: 广播推送
        pushRequest.setTargetValue(messageInfo.getDeviceIds()); //根据Target来设定，如Target=DEVICE, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
...
//        pushRequest.setTargetValue("ALL"); //根据Target来设定，如Target=DEVICE, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
        ...
        // 推送配置
        pushRequest.setTitle(messageInfo.getTitle()); // 消息的标题
        pushRequest.setBody(messageInfo.getMessage()); // 消息的内容
        /*// 推送配置: iOS
        ...
        pushRequest.setAndroidActivity("订单类型"); // 设定通知打开的activity，仅当AndroidOpenType="Activity"有效
